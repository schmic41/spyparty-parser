...
; Behind the scenes
(define (send-all-replays vector-of-good-replays)
  ; Named let to enable custom control flow for venues and dates
  (let recur ([i 0]
              [t-vector vector-of-good-replays]
              [spy-list '()]
              [sniper-list '()]
              [outcome-list '()]
              [venue-list '()]
              [date-list '()])
    (if (= i (vector-length vector-of-good-replays))
...
(define *master-replay-vector* null)
...
(define open-file (new menu-item% [label "Open File(s)"] [parent file-menu]
                       ...
                       [callback (lambda (x y)
                                   (let ([tempfiles (get-file-list
                                     ...
                                     ; Returns #f when dialog cancled, when is guard
                                     (when tempfiles
                                       (begin-busy-cursor)
                                      ...
                                      (callback
                                       (λ (button control-element)
                                         ...
                                         (let* ([p1-name (send filter-p1 get-value)]
                                           ...
                                           (send replay-select set null null null null null)
                                           (let ([temp-sorted (vector-filter temp-filter *master-replay-vector*)])
                                             (set! *temp-replay-vector* temp-sorted)
                                             (send-all-replays *temp-replay-vector*))
                                           (end-busy-cursor))))))
